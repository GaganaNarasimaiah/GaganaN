import React from 'react'


const Navbars = () => {
  return (
    <>
     <div className='firstbar'>
        <nav className='navblock'>
            <div>
                <img src="data:image/png;base64," alt="" />
            </div>
            <div className='content1bloc'>
               <ul>
                <li><h3><a href="">Tvshows</a></h3></li>
               </ul>
               <ul>
                <li><h3><a href="">movies</a></h3></li>
               </ul>
               <ul>
                <li><h3><a href="">search</a></h3></li>
               </ul>
               <ul>
                <li><h3><a href="">account</a></h3></li>
               </ul>
            </div>
        </nav>
    </div>


<div className='secondbar'>
   <nav className='navblock1'>
    <div>
      <img src="https://www.justalternativeto.com/wp-content/uploads/2020/07/Ajio.jpg" alt="" />
    </div>
   <div className='content1bloc1'>
   <ul>
    <li><h3><a href="">search</a></h3></li>
   </ul>
   <ul>
    <li><h3><a href="">orders</a></h3></li>
   </ul>
   <ul>
    <li><h3><a href="">wishlist</a></h3></li>
   </ul>
   <ul>
    <li><h3><a href="">account</a></h3></li>
   </ul>
</div>
</nav>
</div>


<div className='thirdbar'>
   <nav className='navblock2'>
    <div>
      <img src="https://www.internetmatters.org/wp-content/uploads/2018/01/YouTube-logo-pc-2.png" alt="" />
    </div>
   <div className='content1bloc2'>
   <ul>
    <li><h3><a href="">search</a></h3></li>
   </ul>
   <ul>
    <li><h3><a href="">notificaton</a></h3></li>
   </ul>
   <ul>
    <li><h3><a href="">connect</a></h3></li>
   </ul>
   <ul>
    <li><h3><a href="">account</a></h3></li>
   </ul>
</div>
</nav>
</div>

<div className='fourthbar'>
   <nav className='navblock3'>
    <div>
      <img src="https://static.businessworld.in/article/article_extra_large_image/1600858456_HulSrV_Flipkart.jpg" alt="" />
    </div>
   <div className='content1bloc3'>
   <ul>
    <li><h3><a href="">search</a></h3></li>
   </ul>
   <ul>
    <li><h3><a href="">catogories</a></h3></li>
   </ul>
   <ul>
    <li><h3><a href="">favorites</a></h3></li>
   </ul>
   <ul>
    <li><h3><a href="">setting</a></h3></li>
   </ul>
</div>
</nav>
</div>
</>



)
}

export default Navbars;
